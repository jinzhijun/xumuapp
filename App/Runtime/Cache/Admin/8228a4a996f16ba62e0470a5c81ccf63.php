            ...
            <div class="main-container" id="main-container"> 
              ...
              <div>
                <form action="<?php echo u(CONTROLLER_NAME.'/gindex');?>">
                    ...
                    <select name="news_class" style="margin: 0px 0 0 0; float: left;">
                        <?php if(is_array($agree)): $i = 0; $__LIST__ = $agree;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$n_v): $mod = ($i % 2 );++$i;?><option <?php if($n_v['id'] == $_GET['news_class']): ?>selected = selected<?php endif; ?> value="<?php echo ($n_v["id"]); ?>"><?php echo ($n_v["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                          ...
                          <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                              ...
                              <td><span style="color:#999;"><?php echo ($vo["id"]); ?></span></td>
                               <td><?php echo ($vo["class_id"]); ?></td>
                                ...
                                <td class="hidden-480"> <?php if($vo['status'] == 1): ?>非常好<?php elseif($vo['status'] == 2): ?>很好<?php elseif($vo['status'] == 3): ?>好<?php else: ?>差<?php endif; ?></td>
                                <td><div style="height:30px;width:300px; overflow: hidden;">
                                    <?php echo htmlspecialchars_decode($vo['text']); ?>
                                </div></td>
                                <td class=""><?php echo (date("Y-m-d
                                    H:i:s",$vo["edit_date"])); ?></td>
                                <td class="">
                                    <button type="button" class="btn btn-xs btn-danger"
                                                      data-toggle="modal" data-target=".bs-example-modal-sm"> <i class="icon-trash bigger-120 del"
                                                                                                                 data-id="<?php echo ($vo["id"]); ?>"    onclick="return confirmurl('<?php echo u(CONTROLLER_NAME.'/gdelete_data',array('id'=>$vo['id']));?>','您确定要删除吗?')">删除</i> </button></td>
