...
class DaythingController extends BaseController {
    ...
    public function gindex(){
        ...
        $name = I('name');
        if($name){
            $user_where['name'] = $name;
            $user= M('user')->where($user_where)->getField('id',true);
            if($user){
                $users = implode($user);
        ...
        $res =$this->com->order('id asc')->where($where)
            ...
            ->select();
        foreach($res as $k=>$v){
            $class = M('user')->where(array('id'=>$v['user_id']))->find();
            $reply = M('daything_reply')->where(array('thing_id'=>$v['id']))->find();
            $res[$k]['user_id']= $class['name'];
            $res[$k]['info']= $reply['text'];
        }
